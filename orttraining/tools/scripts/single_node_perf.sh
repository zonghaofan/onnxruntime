...
run_perf(){
    ...
    for ((bs=$bs_min; bs<=$bs_max; bs++)); do
        ...
        value=$(./onnxruntime_training_bert --model_name ${model_name} --num_train_steps 100 --train_batch_size ${bs} --optimizer ${optimizer} --max_seq_length ${max_seq_length} --max_predictions_per_seq ${max_predictions_per_seq} --mixed_precision_type=${mixed_precision_type} --mode perf | grep Throughput)
